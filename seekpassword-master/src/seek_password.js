...
function seek_password(hash) {
	...
	for (var i = 0; i <= hash.length - 10; ++i) {
		...
		// validate password
		// Modified by Takuron: Fixed a bug that IE is not support "includes ()".
		var matched = [false, false, false, false];
		sk_pwd.forEach(function(e) {
			matched[0] = matched[0] || lower.indexOf(e) > -1;
			matched[1] = matched[1] || upper.indexOf(e) > -1;
			matched[2] = matched[2] || number.indexOf(e) > -1;
			matched[3] = matched[3] || punctuation.indexOf(e) > -1;
		});
